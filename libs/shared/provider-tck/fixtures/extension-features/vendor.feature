Feature: Vendor-specific behaviour outside the shared contract
  ...
  # the canonical ones share one suite, one backend lifecycle and one report.
  #
  ...
  Scenario: A vendor scenario is gated by the capability declaration like any other
    ...
    # than quietly dropped -- extensions get the gate, the report and the accounting, not just the
    # test runner.
