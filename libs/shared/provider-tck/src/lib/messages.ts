...
const IMPLEMENTATION = '@openfeature/provider-tck';
...
export function readFeatureMessages(uri: string, source: string, newId: IdGenerator.NewId): FeatureMessages {
  ...
  if (failures.length) {
    throw new Error(`provider-tck: ${uri} could not be parsed: ${failures.join('; ')}`);
  }

  const document = envelopes.find((envelope) => envelope.gherkinDocument)?.gherkinDocument;
  if (!document?.feature) {
    throw new Error(`provider-tck: ${uri} contains no Feature`);
  }
  ...
  if (planned.length !== pickles.length) {
    throw new Error(
      `provider-tck: ${uri} compiles to ${pickles.length} scenarios but only ${planned.length} of ` +
...
function rowValues(headers: readonly string[], row: TableRow): Record<string, string> {
  if (row.cells.length !== headers.length) {
    throw new Error(
      `provider-tck: the Examples row at line ${row.location.line} has ${row.cells.length} cells ` +
...
export class ConformanceMessages {
  ...
  record(pickleId: string, initial: Outcome): CompleteTestCase {
    ...
    if (!testCase) {
      throw new Error(
        `provider-tck: no test case was built for pickle ${pickleId}, so its outcome cannot be ` +
