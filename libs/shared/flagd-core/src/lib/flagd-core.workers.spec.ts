 ...
 *
 * They mirror flagd-core.spec.ts exactly but use { workers: true } to exercise
 * the pre-compiled validator and interpreter code paths, proving they avoid
...
describe('flagd-core workers mode resolving', () => {
  describe('truthy variant values', () => {
    ...
    beforeAll(() => {
      core = new FlagdCore(undefined, undefined, { workers: true });
...
describe('flagd-core workers mode targeting evaluations', () => {
  const targetingFlag =
...
describe('flagd-core workers mode validations', () => {
  const mixFlags =
...
describe('flagd-core workers mode common flag definitions', () => {
...
describe('flagd-core workers mode flag metadata', () => {
  const targetingFlag =
...
describe('flagd-core workers mode error conditions', () => {
  const errorFlags = {
    flags: {
      basic: {
  ...
  it('should return a general error code for invalid targeting rule', () => {
    // In interpreter mode (workers: true), targeting rules are NOT compiled at flag-load
    // time — instead, the engine.run() call at evaluation time throws an unknown-operator
    // error. This is caught by FlagdCore.resolve() and surfaces as GENERAL rather than
    // PARSE_ERROR (which is the compiled-mode behavior where the error is caught in the
    // FeatureFlag constructor). This is an accepted behavioral difference between modes.
