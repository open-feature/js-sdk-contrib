...
| `flagChangePollingIntervalMs` | `number`                 | `120000`     | Polling interval for configuration changes. Each delay is jittered by ±10% so that a fleet restarted together does not poll in lockstep. Set to `0` or less to disable polling _(in-process mode only)_ |
